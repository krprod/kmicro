...
/*    @Qualifier("redisTemplateCart")
    @Bean
    public RedisTemplate<String, ProductDTO> redisTemplateCart(RedisConnectionFactory connectionFactory) {
        RedisTemplate<String, ProductDTO> template = new RedisTemplate<>();

        ObjectMapper mapper = createObjectMapper();

        // Pass ObjectMapper directly to the constructor
        Jackson2JsonRedisSerializer<ProductDTO> jackson2JsonRedisSerializer = new Jackson2JsonRedisSerializer<>(mapper, ProductDTO.class);
//        StringRedisSerializer stringSerializer = new StringRedisSerializer();
